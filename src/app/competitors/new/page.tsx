...
import { ComparisonGrid } from '@/app/poten-paper/new/components/visualizations/comparison-grid';

type Step = 'input' | 'generating' | 'result';

const EXAMPLES = [
  '노코드로 이메일 마케팅을 자동화하는 SaaS (타겟: 중소 이커머스)',
  '병원 진료 대기시간을 실시간 표시하는 환자용 앱',
  '중고 명품 AI 감정 + 거래 중개 플랫폼',
];

export default function CompetitorsNewPage() {
  const router = useRouter();
  const { user, loading: authLoading } = useAuth();
  const [step, setStep] = useState<Step>('input');
  const [ideaText, setIdeaText] = useState('');
  const [summary, setSummary] = useState('');
  const [data, setData] = useState<ComparisonGridData | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    if (!user) {
      router.push('/login?next=/competitors/new');
      return;
    }
    ...
    try {
      const response = await fetch('/api/competitors/generate', {
        method: 'POST',
        ...
        >
          <section className="max-w-[720px] mx-auto px-4 sm:px-8 pt-12 pb-6">
            ...
            <p className="text-[14px] text-muted-foreground leading-relaxed">
              사업 아이디어를 입력하면 AI 가 주요 경쟁사를 식별하고, 핵심 기능·특성 기준으로 체크마크 비교표를 작성합니다.
            </p>
          </section>
          ...
          <section className="max-w-[720px] mx-auto px-4 sm:px-8 pb-24">
            ...
            <div className="rounded-2xl border border-border bg-background p-6">
              ...
              <textarea
                ...
                placeholder="예: 노코드 이메일 마케팅 자동화 SaaS..."
                rows={6}
                maxLength={2000}
              ...
              >
                <Sparkles className="w-4 h-4" /> 비교표 생성
              </button>
            </div>
      ...
      {step === 'generating' && (
        ...
        >
          <section className="max-w-[720px] mx-auto px-4 sm:px-8 py-24 text-center">
            ...
            <h2 className="text-xl font-bold text-foreground mb-2">비교표 생성 중...</h2>
            <p className="text-[14px] text-muted-foreground">
              AI 가 경쟁사를 식별하고 기능 비교 중. 약 6초.
            </p>
          </section>
        </motion.div>
      )}
      ...
      {step === 'result' && data && (
        ...
        >
          <div className="max-w-[1156px] mx-auto px-4 sm:px-8 py-10">
            ...
            <ComparisonGrid title="Competitor Comparison" data={data} />
